...
```
Student        { id, matric_no (unique), surname, dob, phone, level, status (active/provisional/graduating/deactivated), deactivation_reason, device_id }
Whitelist      { matric_no, surname, dob, level, session_id, claimed (bool) }
...
```
Admin uploads Whitelist rows (matric_no, surname, dob, level) for the session
Student submits (matric_no, surname/dob) → match against an UNCLAIMED whitelist row
